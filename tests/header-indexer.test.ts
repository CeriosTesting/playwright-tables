...
test.describe("Header Indexer Tests", () => {
  ...
  test("Table header with rowspan throws error", async ({ page }) => {
    await page.goto(
      TestHtmlProvider.getHtmlFilePath(TestHtml.RowspanHeaderTable)
    );

    const headers = HeaderIndexer.HeadersIncludingColspanAndDuplicateAsync(
      page.locator("table>thead>tr>th")
    );
    await expect(headers).rejects.toThrowError(
      "Header with rowspan is not supported."
    );
  });
});
